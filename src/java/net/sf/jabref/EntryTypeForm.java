...
name|helpAction
decl_stmt|;
DECL|method|EntryTypeForm (JabRefFrame frame_, BasePanel panel_, BibtexEntry entry_, JabRefPreferences prefs_)
specifier|public
name|EntryTypeForm
parameter_list|(
name|JabRefFrame
...
name|parent
decl_stmt|;
DECL|method|StoreFieldAction (JDialog parent)
specifier|public
name|StoreFieldAction
parameter_list|(
name|JDialog
...
expr_stmt|;
block|}
comment|/**      * Switches the entry for this editor to the one with the given      * id. If the target entry is of the same type as the current,      * field values are simply updated. Otherwise, a new Dialog is      * created and this one closed.      *      * @param id a<code>String</code> value      */
...
condition|(
literal|true
comment|/*!panel.entryTypeForms.containsKey(id)*/
condition|)
block|{
name|BibtexEntry
name|be
...
argument_list|)
expr_stmt|;
comment|//Util.placeDialog(form, frame);
...
expr_stmt|;
comment|//panel.entryTypeForms.remove(entry.getId());
comment|//panel.entryTypeForms.put(id, form);
block|}
block|}
else|else
block|{
comment|//((EntryTypeForm)(panel.entryTypeForms.get(id)))
...
name|SHORT_DESCRIPTION
argument_list|,
literal|"Generate Bibtexkey (Ctrl-G)"
argument_list|)
expr_stmt|;
