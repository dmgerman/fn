...
parameter_list|)
block|{
comment|/*this.dialog = dialog;         this.status = status;         this.terms = query;         piv = 0;         shouldContinue = true;         parsed = 0;         unparseable = 0;         acmOrGuide = acmButton.isSelected();         fetchAbstract = absCheckBox.isSelected();         int firstEntry = 1;         String address = makeUrl(firstEntry);          try {             URL url = new URL(address);              //dialog.setVisible(true);             String page = getResults(url);              //System.out.println(address);             hits = getNumberOfHits(page, "Found", hitsPattern); 			int index = page.indexOf("Found"); 			if (index>= 0) {             	page = page.substring(index + 5); 				index = page.indexOf("Found"); 				if (index>= 0)             		page = page.substring(index); 			} 			//System.out.println(page);             //System.out.printf("Hit %d\n", hits); 			             if (hits == 0) {                 status.showMessage(Globals.lang("No entries found for the search string '%0'",                         terms),                         Globals.lang("Search ACM Portal"), JOptionPane.INFORMATION_MESSAGE);                 return false;             }              int maxHits = getNumberOfHits(page, "Results", maxHitsPattern);             //System.out.printf("maxHit %d\n", maxHits);             //String page = getResultsFromFile(new File("/home/alver/div/temp50.txt"));              //System.out.println("Number of hits: "+hits);             //System.out.println("Maximum returned: "+maxHits);             if (hits> maxHits)                 hits = maxHits;                          if (hits> MAX_FETCH) {                 status.showMessage(Globals.lang("%0 entries found. To reduce server load, "                         +"only %1 will be downloaded. It will be very slow, in order to make ACM happy.",                                 new String[] {String.valueOf(hits), String.valueOf(MAX_FETCH)}),                         Globals.lang("Search ACM Portal"), JOptionPane.INFORMATION_MESSAGE);                 hits = MAX_FETCH;             }                          parse(dialog, page, 0, firstEntry);         	firstEntry += perPage;         	             while (shouldContinue&& (firstEntry<= hits)) {                 address = makeUrl(firstEntry);                 //System.out.println("Fetch stating at " + firstEntry + " from: " + address);                 page = getResults(new URL(address));                 parse(dialog, page, 0, firstEntry);             	firstEntry += perPage;             }                          return true;                      } catch (MalformedURLException e) {             e.printStackTrace();         } catch (ConnectException e) {             status.showMessage(Globals.lang("Connection to ACM Portal failed"),                     Globals.lang("Search ACM Portal"), JOptionPane.ERROR_MESSAGE);         } catch (IOException e) {         	status.showMessage(Globals.lang(e.getMessage()),                     Globals.lang("Search ACM Portal"), JOptionPane.ERROR_MESSAGE);             e.printStackTrace();         } */
return|return
literal|false
return|;
block|}
...
literal|0
decl_stmt|;
DECL|method|parse (String text, int startIndex, int firstEntryNumber, List<String> entries)
specifier|private
name|void
name|parse
parameter_list|(
...
return|;
block|}
DECL|method|getNextEntryURL (String allText, int startIndex, int entryNumber)
specifier|private
name|String
name|getNextEntryURL
parameter_list|(
