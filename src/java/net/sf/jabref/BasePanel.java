...
argument_list|)
decl_stmt|;
comment|//EntryTypeForm form = new EntryTypeForm
comment|//(frame, ths, be, prefs);
comment|//Util.placeDialog(form, frame); // We want to center the editor.
comment|//form.setVisible(true);
comment|//entryTypeForms.put(id, form);
name|showEntry
argument_list|(
name|be
argument_list|)
...
argument_list|)
decl_stmt|;
comment|//String citeStr="LYXCMD:sampleclient:citation-insert:\\cite{"+(String) model.getValueAt( selectedRows[0], Globals.INDEX_KEY);
name|String
name|citeStr
init|=
...
argument_list|)
expr_stmt|;
comment|//setRightComponent(entryTable.getPane());
name|splitPane
operator|.
...
block|}
block|}
comment|/**      * Shows either normal search results or group search, depending      * on the searchValueField. This is done by reordering entries and      * graying out non-hits.      */
