...
argument_list|()
expr_stmt|;
comment|//CheckBoxFileChooser cb = new CheckBoxFileChooser(new File(""), "Selected only");
comment|//cb.showSaveDialog(frame);
comment|//ExternalFileTypeEditor efte = new ExternalFileTypeEditor(frame);
comment|//efte.setVisible(true);
comment|/*NamedCompound ce = Util.upgradePdfPsToFile(database,                             new String[] {"pdf", "ps"});                     undoManager.addEdit(ce);                     markBaseChanged();*/
block|}
block|}
argument_list|)
expr_stmt|;
