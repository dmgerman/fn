...
argument_list|)
decl_stmt|;
comment|// Show the duplicate resolver dialog if they are deemed duplicates,
comment|// AND they both are still present in the database.
if|if
condition|(
name|eq
operator|&&
...
name|pr
argument_list|(
literal|"BasePanel:1053, removed key bindings"
argument_list|)
expr_stmt|;
comment|//entryTable.getInputMap().put(prefs.getKey("Cut"), "Cut");
comment|//entryTable.getInputMap().put(prefs.getKey("Copy"), "Copy");
comment|//entryTable.getInputMap().put(prefs.getKey("Paste"), "Paste");
name|entryTable
operator|.
name|getActionMap
argument_list|()
