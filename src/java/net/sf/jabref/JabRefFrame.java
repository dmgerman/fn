...
argument_list|,
literal|"Toggle groups inteface"
argument_list|,
literal|"Toggle groups inteface"
argument_list|,
name|GUIGlobals
...
argument_list|)
expr_stmt|;
comment|// If the option is enabled, open the last edited database, if any.
if|if
condition|(
name|prefs
operator|.
...
expr_stmt|;
comment|// End program.
comment|/* 		  if (prefs.getBoolean("openLastEdited")) { 		  // Here we store the current file. If there is no current 		  // file, we remove any previously stored file name. 		  if ((database == null) || (file == null)) 		  prefs.remove("lastEdited"); 		  else 		  prefs.put("lastEdited", file.getPath()); 		  }*/
block|}
block|}
block|}
