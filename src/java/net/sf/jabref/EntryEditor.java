...
argument_list|)
expr_stmt|;
comment|/*if (reqFields == null) { 	    iter = Math.max(optFields.length, genFields.length); 	    rmax = 0; 	    omax = optFields.length; 	} else if (optFields == null) { 	    iter = Math.max(reqFields.length, genFields.length); 	    rmax = reqFields.length; 	    omax = 0; 	} else { 	    iter = Math.max(reqFields.length, optFields.length); 	    rmax = reqFields.length; 	    omax = optFields.length; 	    }*/
name|FieldTextArea
name|ta1
init|=
literal|null
