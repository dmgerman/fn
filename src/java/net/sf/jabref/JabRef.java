...
name|outFormats
init|=
literal|": bibtexml, docbook, endnote, harvard, html, mods, ods, oocalc, simplehtml"
decl_stmt|;
name|int
name|length
init|=
...
argument_list|)
expr_stmt|;
comment|//FileActions.exportDatabase(pr.getDatabase(), data[1],
comment|//    new File(data[0]), pr.getEncoding());
block|}
catch|catch
parameter_list|(
...
return|;
block|}
DECL|method|importFiletypeUnknown (String fname)
name|ParserResult
name|importFiletypeUnknown
parameter_list|(
name|String
name|fname
parameter_list|)
...
expr_stmt|;
block|}
comment|/**      * Morten Alver (sept. 06): I'm adding this to run the functionality for      * OpenOffice integration. I'm not sure yet how to handle deployment issues,      * because the OO stuff requires several OO jars on the classpath, which we      * shouldn't distribute because OO installations already have them.      *      * To avoid introducing dependencies on these jars for the time being,      * I'm keeping OO "plugin" files separate, only invoking them through reflection.      *      * @param jrf The JabRefFrame.      */
DECL|method|startOOPlugin (JabRefFrame jrf)
specifier|private
name|void
name|startOOPlugin
parameter_list|(
...
name|forName
argument_list|(
literal|"net.sf.jabref.oo.OOTestPanel"
argument_list|)
decl_stmt|;
name|Method
name|m
...
block|{
comment|// Do nothing.
comment|//System.out.println("OO plugin not found.");
