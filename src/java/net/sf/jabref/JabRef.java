...
parameter_list|)
block|{
comment|/*         String aut = "{Bill and Bob Alver}, Jr., Morten Omholt and Alfredsen, Jo A. and Ãie, G. and Yngvar von Olsen";         System.out.println("lastnameFirst: "+ImportFormatReader.fixAuthor_lastNameFirst(aut));         System.out.println("lastnameFirstCommas: "+ImportFormatReader.fixAuthor_lastNameFirstCommas(aut,false));         System.out.println("lastnameFirstCommas (abbr): "+ImportFormatReader.fixAuthor_lastNameFirstCommas(aut,true));         System.out.println("firstNameFirst: "+ImportFormatReader.fixAuthor_firstNameFirst(aut));         System.out.println("firstNameFirstCommas: "+ImportFormatReader.fixAuthor_firstNameFirstCommas(aut, false));         System.out.println("forAlphabetization: "+ImportFormatReader.fixAuthorForAlphabetization(aut));         */
name|ths
operator|=
name|this
expr_stmt|;
...
name|INVALID_FORMAT
condition|)
comment|// We will try to import this file instead:
name|toImport
operator|.
name|add
argument_list|(
...
argument_list|)
decl_stmt|;
comment|/*if (data.length == 1) {                 // Load a bibtex file:                 ParserResult pr = openBibFile(data[0]);                  if (pr != null)                     loaded.add(pr);             } else if (data.length == 2) {*/
...
argument_list|)
decl_stmt|;
comment|/*if (data.length == 1) {                 // Load a bibtex file:                 ParserResult pr = openBibFile(data[0]);                  if (pr != null) {                     pr.setToOpenTab(true);                     loaded.add(pr);                 }             } else if (data.length == 2) {*/
comment|// Import a database in a certain format.
try|try
block|{
if|if
