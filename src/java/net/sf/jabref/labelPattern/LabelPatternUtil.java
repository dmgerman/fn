...
index|]
expr_stmt|;
comment|//int _mi = val.indexOf(":");
comment|//if(_mi != -1&& _mi != val.length()-1&& _mi != 0) { // ":" is in val and isn't first or last character
comment|//modifier=val.substring(_mi+1);
comment|//val=val.substring(0,_mi);
comment|//}
name|StringBuffer
name|_sbvalue
init|=
...
condition|)
block|{
comment|// For label code "auth...": if there is no author, but there are editor(s)
comment|// (e.g. for an Edited Book), use the editor(s) instead. (saw27@mrao.cam.ac.uk).
comment|// This is what most people want, but in case somebody really needs a field which
comment|// expands to nothing if there is no author (e.g. someone who uses both "auth"
comment|// and "ed" in the same label), we provide an alternative form "pureauth..." which
comment|// does not do this fallback substitution of editor.
name|String
name|authString
decl_stmt|;
if|if
...
condition|)
block|{
comment|// remove the "pure" prefix so the remaining code in this section functions correctly
name|val
operator|=
name|val
operator|.
