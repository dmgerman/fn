...
init|=
literal|"uno:socket,host=localhost,port=2002;urp;StarOffice.ServiceManager"
decl_stmt|;
DECL|field|BIB_TYPES
specifier|final
name|String
index|[]
name|BIB_TYPES
...
return|;
block|}
DECL|method|testCustomProperties ()
specifier|public
name|void
...
name|addProperty
argument_list|(
literal|"JR_cite_1_Danielsen1987"
argument_list|,
operator|(
name|short
operator|)
...
argument_list|)
decl_stmt|;
comment|//System.out.println(text+" / "+xViewCursor.getText());
name|xViewCursor
operator|.
name|getText
argument_list|()
...
name|type
expr_stmt|;
comment|// Remember the type in case we need to uniqiefy.
name|String
index|[]
name|keys
init|=
...
block|}
block|}
comment|/*System.out.println(style.getBooleanCitProperty("MultiCiteChronological"));                     for (int j = 0; j< cEntries.length; j++) {                         BibtexEntry cEntry = cEntries[j];                         System.out.println(cEntry.getCiteKey());                     } */
name|citationMarker
operator|=
name|style
operator|.
...
name|Exception
block|{
comment|// Check if there already is a bookmarked section:
...
name|println
argument_list|(
literal|"Found existing JabRef bookmark"
argument_list|)
expr_stmt|;
return|return
literal|true
...
block|}
block|}
DECL|method|clearBibTextSectionContent ()
specifier|public
name|void
name|clearBibTextSectionContent
parameter_list|()
throws|throws
name|Exception
block|{
comment|// Get a range comparator:
...
argument_list|)
decl_stmt|;
comment|// Find the bookmarks for the bibliography:
name|XTextRange
name|range
init|=
name|getBookmarkRange
...
condition|)
block|{
comment|// No end bookmark. This means that there is no bibliography.
return|return;
block|}
comment|// Get a paragraph cursor at the start of the bibliography:
comment|//System.out.println("text="+text+" range="+range);
name|XTextCursor
name|mxDocCursor
init|=
name|text
...
expr_stmt|;
block|}
comment|// Finally, clear the bibliography:
name|mxDocCursor
operator|.
name|setString
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// If we lost the start bookmark, recreate it:
if|if
condition|(
name|getBookmarkRange
argument_list|(
...
name|createInstance
argument_list|(
literal|"com.sun.star.text.Footnote"
argument_list|)
argument_list|)
decl_stmt|;
name|xFootnote
...
argument_list|)
expr_stmt|;
comment|// doesn't work: short data type
name|System
operator|.
name|out
operator|.
...
return|;
block|}
DECL|method|printBookmarkNames ()
specifier|public
name|void
name|printBookmarkNames
parameter_list|()
throws|throws
name|Exception
block|{
...
block|}
block|}
comment|/**      * Focus the active OO document.      */
...
block|}
block|}
DECL|method|testFrameHandling ()
specifier|public
name|void
name|testFrameHandling
parameter_list|()
throws|throws
name|Exception
block|{
...
literal|true
expr_stmt|;
comment|// argument xModel wins over URL.
name|System
operator|.
name|out
operator|.
...
argument_list|(
literal|"private:factory/swriter"
argument_list|,
literal|"_blank"
argument_list|,
literal|0
...
argument_list|)
expr_stmt|;
comment|// Controller of the hidden frame
name|XController
name|xController
init|=
name|newDoc
...
argument_list|()
decl_stmt|;
comment|//xContainerWindow.setVisible(true);
comment|//xComponentWindow.setFocus();
comment|//xContainerWindow.setVisible(false);
name|xFrame
operator|.
name|dispose
argument_list|()
