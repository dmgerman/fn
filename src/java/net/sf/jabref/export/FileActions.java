...
literal|0
decl_stmt|,
DECL|field|COPIED_TO_TEMP
name|COPIED_TO_TEMP
init|=
literal|1
decl_stmt|,
DECL|field|COPIED_TO_BACKUP
name|COPIED_TO_BACKUP
init|=
literal|2
decl_stmt|,
DECL|field|INIT_OK
...
block|}
block|}
DECL|method|repairAfterError (File file, boolean backup, int status)
specifier|public
specifier|static
name|void
...
condition|)
block|{
comment|// Nothing has happened except for possibly creating the temp file.
if|if
condition|(
name|temp
operator|.
...
condition|)
block|{
comment|// We may have begun copying to the backup file. Try to restore it from temp:
if|if
condition|(
name|temp
operator|.
...
condition|)
block|{
comment|// Seems we couldn't delete the temp file. This is probably a very unlikely scenario.
comment|// Try to copy back:
name|Util
operator|.
name|copyFile
argument_list|(
...
block|{
comment|// INIT_OK
comment|// Something happened after initializing, when saving. Try to restore the file from the
comment|// backup (if active) or temp file:
if|if
condition|(
name|backup
condition|)
...
block|}
block|}
comment|/*     if (file.exists()) { 	    file.delete(); 	}  	if (temp.exists()) {         Util.copyFile(temp, file, true);         temp.delete(); 	    //temp.renameTo(file); 	} 	else {         Util.copyFile(back, file, true);         back.delete(); 	    //back.renameTo(file); 	}   */
block|}
...
argument_list|)
decl_stmt|;
comment|// Make room for the save:
name|int
name|status
init|=
name|NOTHING
