...
return|;
block|}
comment|/**      * The log event will be forwarded to the {@link LogMessages} archive.      *<p/>      * All logged messages will pass this method. For each line in the messages of the log event a new log event will be created.      *      * @param event log event      */
annotation|@
name|Override
...
argument_list|)
expr_stmt|;
comment|// stack traces logged by 'Log.error("message"), e' will be split by new lines so we can create a new log event for each line as 'e.printStackTrace()' would do.
if|if
condition|(
name|event
operator|.
