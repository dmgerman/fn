...
argument_list|()
decl_stmt|;
comment|// int answer = DuplicateResolverDialog.resolveDuplicateInImport
comment|//        (frame, duplicate, entry);
...
block|}
block|}
comment|// Remove the old entry, if we are supposed to
if|if
condition|(
name|remove
condition|)
...
expr_stmt|;
block|}
comment|// Add the merged entry, if we are supposed to:
if|if
condition|(
name|mergedEntry
operator|!=
...
name|lang
argument_list|(
literal|"Added %0 entries. Replaced %1 entries. Added information to %2 entries."
argument_list|,
name|addCount
operator|.
name|toString
