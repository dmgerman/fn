...
argument_list|)
expr_stmt|;
name|setStringRepresenationFromFile
argument_list|(
name|result
argument_list|)
expr_stmt|;
...
return|;
block|}
DECL|method|setStringRepresenationFromFile (BibtexEntry entry)
specifier|private
name|void
name|setStringRepresenationFromFile
parameter_list|(
...
decl_stmt|;
name|boolean
name|currentLineBelongsToEntry
init|=
literal|false
decl_stmt|;
for|for
...
control|)
block|{
comment|//if we find the key, the entry starts
if|if
condition|(
name|line
operator|.
...
literal|true
expr_stmt|;
comment|//if we find a new entry, the current one stops. Whitespaces after an entry belong to the entry
block|}
elseif|else
if|if
condition|(
