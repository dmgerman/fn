...
argument_list|)
expr_stmt|;
comment|//fetch the raw Bibtex results from IEEEXplore and parse
name|URL
name|bibtexURL
init|=
operator|new
...
comment|//Pattern escapedPattern = Pattern.compile("(?<!&)#([x]*)([0]*)(\\p{XDigit}+);");
name|bibtexPage
operator|=
name|bibtexPage
operator|.
...
expr_stmt|;
block|}
DECL|method|makeSearchPostRequestPayload (int startIndex)
specifier|private
name|String
name|makeSearchPostRequestPayload
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|//author = author.replaceAll("([^;]+),([^;]+),([^;]+)", "$1,$3,$2"); // Change order in case of Jr. etc
name|author
operator|=
name|author
operator|.
