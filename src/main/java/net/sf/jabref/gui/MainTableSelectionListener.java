...
expr_stmt|;
comment|// TODO SIMON
comment|//panel.frame().getSearchBar().addSearchListener(previewPanel[0]);
comment|//panel.frame().getSearchBar().addSearchListener(previewPanel[1]);
name|this
operator|.
name|preview
operator|=
...
block|}
block|}
comment|//If there are no file types specified, consider all files.
elseif|else
if|if
condition|(
name|fileList
...
condition|)
block|{
comment|//                            if (fileList.getRowCount()> 0) {
comment|//                                FileListEntry flEntry = fileList.getEntry(0);
name|ExternalFileMenuItem
name|item
init|=
operator|new
