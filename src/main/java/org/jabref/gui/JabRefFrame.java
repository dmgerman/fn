...
argument_list|)
decl_stmt|;
DECL|field|jabrefTwitterAction
specifier|private
specifier|final
name|AbstractAction
name|jabrefTwitterAction
init|=
operator|new
name|OpenBrowserAction
argument_list|(
...
argument_list|)
decl_stmt|;
comment|//close.addActionListener(closeDatabaseAction);
name|closeOthers
operator|.
...
argument_list|)
decl_stmt|;
comment|// databasePropertiesMenu.addActionListener(this.databaseProperties);
name|popupMenu
operator|.
name|add
argument_list|(
...
block|{
comment|// TODO: popup
comment|// tabbedPane = new DragDropPopupPane(tabPopupMenu());
name|initSidePane
argument_list|()
expr_stmt|;
name|initLayout
...
expr_stmt|;
comment|// TODO: Color
comment|//tabbedPane.setForeground(GUIGlobals.INACTIVE_TABBED_COLOR);
...
argument_list|)
expr_stmt|;
comment|// groupSidePane.getToggleAction().setSelected(sidePaneManager.isComponentVisible(GroupSidePane.class));
comment|//previewToggle.setSelected(Globals.prefs.getPreviewPreferences().isPreviewPanelEnabled());
comment|//generalFetcher.getToggleAction().setSelected(sidePaneManager.isComponentVisible(GeneralFetcher.class));
comment|//openOfficePanel.getToggleAction().setSelected(sidePaneManager.isComponentVisible(OpenOfficeSidePanel.class));
...
argument_list|)
decl_stmt|;
comment|// TODO: Remove this hack as soon as toolbar is implemented in JavaFX and these events are no longer captured globally
name|tabbedPaneContainer
operator|.
name|addKeyListener
argument_list|(
...
parameter_list|)
block|{
comment|// We need to consume a few events that have a global listener
comment|// Otherwise, they propagate to the JFrame (i.e. "Ctrl + A" in the entry editor still triggers the "Select all" action)
name|Optional
argument_list|<
name|KeyBinding
argument_list|>
...
decl_stmt|;
comment|// TODO:
comment|//tabbedPane.setTransferHandler(xfer);
name|tlb
operator|.
...
argument_list|)
expr_stmt|;
comment|// TODO: mb.setTransferHandler(xfer);
name|sidePaneManager
operator|.
name|getPanel
argument_list|()
...
block|}
block|}
DECL|method|sortTabs ()
specifier|public
name|void
name|sortTabs
parameter_list|()
block|{
comment|// We are going to be crude: remove all tabs and re-add them in a sorted way
comment|// This is ugly, but otherwise we run into issues: https://stackoverflow.com/questions/37328760/javafx-tabpane-sorting-tabs-creates-havoc
name|List
argument_list|<
name|Tab
argument_list|>
