...
argument_list|()
decl_stmt|;
comment|// reference layout mapped from entry type:
...
expr_stmt|;
block|}
DECL|method|getDefaultBibLayout ()
specifier|public
name|Layout
...
block|}
block|}
DECL|method|getReferenceFormat (EntryType type)
specifier|public
name|Layout
...
return|;
block|}
comment|/**      * This method looks up a field for an entry in a database. Any number of backup fields can be used      * if the primary field is empty.      *      * @param entry    The entry.      * @param database The database the entry belongs to.      * @param fields   The field, or succession of fields, to look up. If backup fields are needed, separate      *                 field names by /. E.g. to use "author" with "editor" as backup, specify StandardField.orFields(StandardField.AUTHOR, StandardField.EDITOR).      * @return The resolved field content, or an empty string if the field(s) were empty.      */
DECL|method|getCitationMarkerField (BibEntry entry, BibDatabase database, String fields)
specifier|private
name|String
name|getCitationMarkerField
parameter_list|(
